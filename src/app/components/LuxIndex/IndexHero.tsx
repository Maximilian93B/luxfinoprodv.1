...
export default function HeroIndex({ openQuoteDrawer }: { openQuoteDrawer: () => void }) {
  ...
  return (
    <section className="relative h-screen overflow-hidden bg-luxcopper">
      <motion.div
        className="absolute inset-x-0 top-0 z-30 pt-24 sm:pt-32 md:pt-40 lg:pt-48 pb-8 bg-gradient-to-b from-luxcopper/90 to-transparent"
      ...
      >
        <div className="max-w-7xl mx-auto flex flex-col items-center justify-center px-4 sm:px-6 lg:px-8">
          <motion.h1
            className="text-5xl sm:text-5xl md:text-5xl lg:text-7xl font-bold text-center mb-2 sm:mb-4 font-playfair text-luxpearl drop-shadow-lg"
            initial={{ opacity: 0, y: -20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, delay: 0.4 }}
          >
            <span className="text-luxcedar">Welcome to</span>
            <br />
            <span className="text-luxpearl">LuxFino</span>
          </motion.h1>
          <motion.p
            className="text-lg sm:text-lg md:text-xl lg:text-2xl text-luxpearl font-avenir text-center drop-shadow-md"
            initial={{ opacity: 0, y: -20 }}
            animate={{ opacity: 1, y: 0 }}
      ...
      <div className={`h-full flex ${isMobile ? 'flex-col' : 'flex-row'} mt-20 sm:mt-0`}>
